...
const ComparePage = () => {
    ...
    if (!diff) {
        return <div className="text-center text-gray-400 mt-20">No diff data found. Please upload ZIPs on the home page.</div>;
    }
    ...
    const allFiles = [
        ...
        ...diff.modified.map(f => ({ path: f, status: "modified" as FileStatus })),
        ...diff.unchanged.map(f => ({ path: f, status: "unchanged" as FileStatus })),
    ];
    ...
    return (
        <main className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 transition-colors">
            <div className="w-full max-w-6xl p-8 bg-white dark:bg-gray-800 rounded-lg shadow-md mt-12 relative">
                ...
                <h1 className="text-2xl font-bold mb-6 text-center text-gray-900 dark:text-gray-100 tracking-tight">Compare ZIP Files</h1>
                <div className="mb-8">
                    <DiffSummaryBar added={diff.added.length} removed={diff.removed.length} modified={diff.modified.length} />
                </div>
                <div className="flex flex-col md:flex-row gap-10">
                    <section className="flex-1">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-900 border rounded p-4 min-h-[200px] max-h-[400px] overflow-auto shadow-inner">
                            <FileTreeView files={allFiles} onSelect={setSelectedFile} selected={selectedFile} />
                        </div>
                    </section>
                    ...
                    <section className="flex-1">
                        ...
                        <div className="bg-gray-100 dark:bg-gray-700 rounded p-4 min-h-[120px] max-h-[400px] overflow-auto shadow-inner">
                            <DiffDetailsPanel diff={selectedFile && diff.diffs[selectedFile] ? diff.diffs[selectedFile] : "Select a file to view diff."} />
                        </div>
                    </section>
                </div>
            </div>
